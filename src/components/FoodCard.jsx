...
const FoodCard = ({ food }) => {
  ...
  return (
    <Link to={`/food/${food.id}`}>
      <div className="bg-white/90 dark:bg-gray-800/90 shadow-lg rounded-2xl p-4 w-64 h-[370px] flex flex-col justify-between transition-colors duration-300 border border-gray-300 dark:border-gray-700 text-black dark:text-white">
        ...
        <div className='relative'>
          ...
          >
            <Heart className={`w-6 h-6 transition-all ${isFav ? 'text-red-500' : 'text-white/90'}`} fill={isFav ? 'red' : 'none'} />
          </button>
        </div>
        ...
        <div className="w-full max-w-xs mx-auto h-px bg-gray-400/90 dark:bg-white/20 my-1"></div>
        
        <span className="text-sm text-green-700 dark:text-green-400 h-10 overflow-hidden block">
